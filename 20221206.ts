...
function queue(queuers: any[], pos: number): number {
  let minutes = 0;
  let ticketsNeeded = queuers[pos];
  queuers[pos] = "ME";

  while (ticketsNeeded > 0) {
    let currentUser = queuers.shift();
    minutes++;
    if (currentUser === "ME") {
      ticketsNeeded--;
      if (ticketsNeeded === 0) break;
      queuers.push("ME");
    } else if (currentUser - 1 > 0) {
      queuers.push(currentUser - 1);
    }
  }
